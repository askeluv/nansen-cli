...
describe('assertHlBridgeAuthorizeIntent', () => {
  ...
  it('passes when wallet/depositor equal the signer (case-insensitive)', () => {
    const value = nonceMapping();
    value.wallet = WALLET.toLowerCase();
    expect(() => assertHlBridgeAuthorizeIntent(value, WALLET)).not.toThrow();
  });

  it('throws SIGNER_MISMATCH when wallet differs from the signer', () => {
    const value = { ...nonceMapping(), wallet: '0x' + '11'.repeat(20) };
    expect(() => assertHlBridgeAuthorizeIntent(value, WALLET)).toThrow(/SIGNER_MISMATCH|but the signing wallet is/);
    try {
      assertHlBridgeAuthorizeIntent(value, WALLET);
    } catch (err) {
  ...
  it('throws SIGNER_MISMATCH when depositor differs from the signer', () => {
    const value = { ...nonceMapping(), depositor: '0x' + '22'.repeat(20) };
    expect(() => assertHlBridgeAuthorizeIntent(value, WALLET)).toThrow(/but the signing wallet is/);
  });

  it('is a no-op when wallet/depositor fields are absent', () => {
    expect(() => assertHlBridgeAuthorizeIntent({ chainId: 'hyperliquid', nonce: 1 }, WALLET)).not.toThrow();
  });
});
