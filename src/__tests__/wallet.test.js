...
describe('Path traversal security: wallet delete command', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it('rejects path traversal in delete command with ../ sequences', async () => {
    // Create a legitimate wallet first
    createWallet('legitimate', null);
    ...
    expect(errors.length > 0 || logs.some(l => l.includes('not found'))).toBe(true);
  });

  it('rejects absolute paths in delete command', async () => {
    createWallet('safe-wallet', null);
  ...
  it('allows legitimate wallet deletion without path traversal', async () => {
    createWallet('safe-delete', null);
    ...
    await cmds.wallet(['delete'], null, {}, { name: 'safe-delete' });
    
    expect(logs.some(l => l.includes('deleted'))).toBe(true);
  });
});

describe('Path traversal security: wallet send command', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it('rejects path traversal in send command wallet option', async () => {
    createWallet('sender', null);
    ...
    // Attempt path traversal via --wallet option
    await cmds.wallet(['send'], null, { 'dry-run': true }, {
      wallet: '../../etc/passwd',
      to: '0x742d35Cc6bF4F3f4e0e3a8DD7e37ff4e4Be4E4B4',
      amount: '0.01',
    ...
    // Should fail due to invalid wallet path
    expect(logs.length === 0 || logs.some(l => l.includes('not found') || l.includes('Invalid'))).toBe(true);
  });

  it('allows legitimate send with valid wallet name', async () => {
    createWallet('sender-safe', null);
    ...
    // This will fail due to missing RPC setup, but path validation should pass
    await cmds.wallet(['send'], null, { 'dry-run': true }, {
      wallet: 'sender-safe',
      to: '0x742d35Cc6bF4F3f4e0e3a8DD7e37ff4e4Be4E4B4',
      amount: '0.01',
    ...
    expect(logs.every(l => !l.includes('Invalid path'))).toBe(true);
  });
});
