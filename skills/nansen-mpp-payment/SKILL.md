...
- X Layer USDT0 x402 is supported via the OKX facilitator. The CLI-side EIP-712 domain separator is correct. If you see `invalid_exact_evm_signature` for X Layer, verify the server deployment includes the OKX facilitator (check `x402_xlayer_wallet` / OKX env vars). Use Base USDC as a fallback.
