...
- **x402** (micropayment, native to this CLI) — `nansen wallet create` + fund with USDC on Base or Solana. The CLI auto-signs the `Payment-Signature` header on 402 responses. See `src/x402.js` and the `--x402-payment-signature` flag.
